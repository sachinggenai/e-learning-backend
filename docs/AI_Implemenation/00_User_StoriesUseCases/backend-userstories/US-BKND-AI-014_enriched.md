## US-BKND-AI-014 — Support Simple Chat Edit Scenario
...
- **Priority:** MUST
- **Depends on:** US-BKND-AI-007 (Page List and Fetch Tools), US-BKND-AI-012 (Update Page Proposal and Apply)
- **Unlocks:** US-BKND-AI-022 (E2E Regression Suite), US-BKND-AI-030 (Course Assembly into Editor)
...
- Instructions that include PII or prompt injection: returns a safety violation error (per US-BKND-AI-025).
...
The entire chat endpoint is gated by `AI_AUTHORING_ENABLED` (from US-BKND-AI-002). When disabled:
...
|---|---|---|---|
| `session_id` | string (UUID) | Yes | Active AI session from US-BKND-AI-006 |
...
class ChatOrchestrator:
    ...
    async def process_turn(
        ...
        try:
            ...
            # 3. Get the model (primary with fallback per US-BKND-AI-026)
...
- Model provider fallback (US-BKND-AI-026) ensures continuity if primary fails.
...
- Prompt injection scanning at intake (US-BKND-AI-025 integration point).
...
|---|---|
| Page CRUD repositories | US-BKND-AI-007 (list_pages, fetch_page) via existing `PageRepository` |
| Update page proposal | US-BKND-AI-012 (propose_update_page, apply_update_proposal) |
| Proposal lifecycle | US-BKND-AI-009 (generic proposal lifecycle, states, TTL) |
| Apply safety, audit, outbox | US-BKND-AI-010 (transactional apply, audit write, outbox event) |
| Validation pipeline | US-BKND-AI-008 (schema, business rules, export, accessibility) |
| AI session management | US-BKND-AI-006 (session create, validate, expire, scope) |
| Model routing and fallback | US-BKND-AI-026 (primary/fallback model, circuit breaker) |
| Feature flags and config | US-BKND-AI-002 (AI_AUTHORING_ENABLED, model config, env vars) |
...
|---|---|---|
| Full course creation from file | US-BKND-AI-019 | Requires file ingestion pipeline (US-BKND-AI-016, US-BKND-AI-017) |
| Batch proposal operations | US-BKND-AI-029 | Chat edit is single-page; batch is multi-page |
| Streaming responses (SSE) | US-BKND-AI-023 | Chat returns complete response; streaming is future |
| Real-time job status polling | US-BKND-AI-045 | Not needed: chat edits are synchronous |
| Course-level AI operations | US-BKND-AI-046 | Chat edit is page-level; course create/delete separate |
| Multi-user collaboration | US-BKND-AI-047 | Session is single-user; collaboration is future |
| Template harvesting | US-BKND-AI-037 | Chat edit uses existing templates, does not create new ones |
| Context window recovery | US-BKND-AI-039 | Detection and refresh of overflowing context mid-turn |
| JSON repair | US-BKND-AI-027 | LLM output repair; useful but not gating for MVP |
| A/B prompt testing | US-BKND-AI-042 | Prompt versioning and cohort assignment is separate |
...
| T-017 | Integrate prompt safety scanning (US-BKND-AI-025 interface point) | `app/routers/ai_chat.py` | T-016 |
