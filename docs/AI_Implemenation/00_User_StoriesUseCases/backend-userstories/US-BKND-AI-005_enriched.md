# US-BKND-AI-005: Version Tool Schemas and Template Contracts

**Priority:** MUST  
**Depends on:** US-BKND-AI-003 (Create Isolated AI API Module)  
...
2. Backend validates session exists and is active (FR-2 from US-BKND-AI-003)
...
- The propose-apply confirmation modal already defined in UI/UX for US-BKND-AI-003.
...
- Tool validation endpoints are session-scoped (requires valid session from US-BKND-AI-003).
...
| Tool Validate router (or extend existing) | Add validate endpoint to existing `app/routers/ai_tools.py` (from US-BKND-AI-003) |
...
| `app/services/ai/__init__.py` | Export new service classes | No existing imports break; this file currently exports nothing AI-specific (it may not even exist yet; US-BKND-AI-003 creates the `app/services/ai/` package) |
| Alembic `env.py` | Ensure new models are imported for autogenerate | Must not alter existing table detection |
| (future) `app/services/ai/prompt_builder.py` | Will consume `ToolRegistry.get_system_prompt_tools_block()` at session creation | This is a future integration point (US-BKND-AI-006), not this story |

**Critical non-regression:** The existing `POST /api/v1/ai/tools/propose_create_page` (from US-BKND-AI-003) must still work after this story. It will internally call the new `TemplateValidationEngine.validate()` instead of inline validation. The response shape must be identical -- this is a refactoring of the validation internals, not the public API.
...
|---|---|---|---|---|
| **T1** | **Create database schema (migration + models)** | Backend Engineer | 6 | US-BKND-AI-003 complete (for AI module structure) |
...
| T5.3 | Integrate validation engine into `propose_create_page` and `propose_update_page` handlers (from US-BKND-AI-003) | Backend Engineer | 2 | T4.3, US-BKND-AI-003 |
...
**Critical Path:** T1 -> T3 -> T4 -> T5 -> T7 (validation engine is on the critical path because it blocks the propose/apply tool feature from US-BKND-AI-003). T2 can be done in parallel with T3. T6 can be done in parallel with T5. T8 can be done in parallel with T7.
