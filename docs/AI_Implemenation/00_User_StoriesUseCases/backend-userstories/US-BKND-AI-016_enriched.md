# US-BKND-AI-016 -- Add File Upload and Ingestion Job Foundation for AI
...
- **Depends on:** US-BKND-AI-004 (AI persistence foundations) -- requires `ai_ingestion_jobs` table alongside the existing `import_jobs` table
...
The platform MUST provide a dedicated document upload and ingestion endpoint that accepts PDF, DOCX, and configurable package types (ZIP/SCORM), creates a durable ingestion job with a polling URL, performs deterministic extraction of text content and structure, and stages the extracted payload for downstream AI segmentation (US-BKND-AI-017) and course generation (US-BKND-AI-019). The ingestion flow MUST be decoupled from the existing SCORM import pipeline at the router level but MAY reuse `ImportService` and `StrategyRegistry` for ZIP/SCORM packages behind the scenes.
...
CREATE TABLE ai_ingestion_jobs (
    ...
    session_id      VARCHAR(64),                                -- FK: ai_sessions.job_id (US-BKND-AI-004)
...
```python
"""ORM models for AI module persistence (US-BKND-AI-004, US-BKND-AI-016, etc.)."""
...
```ini
# ============================================
# AI Ingestion Settings (US-BKND-AI-016)
# ============================================
...
5. **Session scoping:** When `session_id` is provided, the ingestion job is tied to that AI session. `GET` and `DELETE` should verify the caller's session matches (deferred to US-BKND-AI-017's authorization pass).
...
| Session-bound ingestion | No session context | `session_id` FK (schema only; enforcement in US-BKND-AI-017) |
...
### 5.1 Immediate (MVP+1 -- US-BKND-AI-017)
...
| Page plan preview endpoint | `GET /ai/ingestions/{job_id}/plan` returning the AI-segmented page breakdown (LLM call in US-BKND-AI-017). |
...
|-------------|-------|---------------|
| AI session binding | US-BKND-AI-004 | `ai_ingestion_jobs.session_id` is an FK to `ai_sessions.job_id` (schema in this story; enforce in US-BKND-AI-017) |
| Page plan / segmentation | US-BKND-AI-017 | `GET /ai/ingestions/{job_id}/plan` consumed by LLM segmenter; uses `extracted_sections` as input |
| Batch proposal apply | US-BKND-AI-019 | Ingestion job ID stored on batch proposal for provenance |
| Durable workflow engine | US-BKND-AI-034 | Ingestion becomes first phase of `FileIngestionWorkflow` |
| Cost tracking | US-BKND-AI-036 | Log ingestion file_size and extraction duration as billable units |
