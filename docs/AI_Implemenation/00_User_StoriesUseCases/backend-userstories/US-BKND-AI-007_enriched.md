# US-BKND-AI-007 -- List and Fetch Course Pages for AI Context
...
**Priority:** MUST  
**Depends on:** US-BKND-AI-006 (AI Session Creation)  
**Unlocks:** US-BKND-AI-008 (Validation Pipeline), US-BKND-AI-011 (Create Page Proposal), US-BKND-AI-012 (Update Page Proposal), US-BKND-AI-014 (Chat Edit)
...
6. **FR-6 -- Staleness Detection:** Every page response (both list and fetch) MUST include a `page_etag` field. This is used by subsequent proposal calls (US-BKND-AI-008/009) to detect concurrent modifications. The etag is computed as a SHA-256 hash of the page record's JSON serialization.
...
```
Step 1:  AI Agent constructs list_pages call with session_id from US-BKND-AI-006.
...
These are backend AI tool endpoints -- there is no direct human UI interaction. However, the frontend AI integration layer (US-BKND-AI-024) MUST:
...
The existing pages and components tables are reused (no new DDL needed for this story). The session table is defined in US-BKND-AI-006. The `page_etag` is computed at the application layer, not stored in the database.
...
**AI Sessions table (defined in US-BKND-AI-006, referenced here):**
```sql
...
**AI Audit Logs table (referenced but not created by this story; created by US-BKND-AI-004):**
```sql
...
class ToolExecutor:
    ...
    def __init__(
        self,
        session_repo: "AISessionRepository",         # from US-BKND-AI-006
        page_repo: PageRepository,
        audit_repo: "AIAuditLogRepository",           # from US-BKND-AI-004
        rate_limiter: "RateLimiter",                   # from US-BKND-AI-021
    ):
...
async def _get_executor(session: AsyncSession = Depends(get_session)) -> ToolExecutor:
    from app.repositories.page_component_repo import PageRepository
    # Repositories injected here; session_repo and audit_repo from US-BKND-AI-004/US-BKND-AI-006
...
|---|---|---|
| `AISessionRepository` | US-BKND-AI-006 (to be built) | Validate session, look up course_id |
| `PageRepository` | `app/repositories/page_component_repo.py` | List/fetch page records (EXISTS) |
| `AIAuditLogRepository` | US-BKND-AI-004 (to be built) | Write audit trail |
| `RateLimiter` | US-BKND-AI-021 (to be built) | Enforce per-user/per-hour tool call limits |
...
| `AISessionRepository.get_active(session_id)` | Returns active session or None | NEW (US-BKND-AI-006) |
| `AIAuditLogRepository.log(...)` | Writes audit entry | NEW (US-BKND-AI-004) |
| `RateLimiter.check(user_id, tool_name)` | Returns (allowed, retry_after) | NEW (US-BKND-AI-021) |
...
**Authorization Model:**
- Session-based: the caller presents a session_id obtained from POST /api/v1/ai/sessions (US-BKND-AI-006).
...
| `app/repositories/ai_session_repo.py` | Session persistence (referenced as dependency, created by US-BKND-AI-006). Placeholder or stub needed for testing. |
| `app/repositories/ai_audit_repo.py` | Audit logging (referenced as dependency, created by US-BKND-AI-004). Placeholder or stub needed for testing. |
| `app/services/ai/rate_limiter.py` | Rate limiter (referenced as dependency, created by US-BKND-AI-021). Simple in-memory stub for MVP. |

### 4.3 What Existing Code Must Be Modified

| File | Change | Non-Regression Constraint |
|---|---|---|
| `app/main.py` | Add `from app.routers import ai_tools` and `api_router.include_router(ai_tools.router)` | Existing route registration is unchanged; disabling AI must still work if the feature flag is off (US-BKND-AI-002). The import should be behind a feature flag or conditionally registered. |
...
3. **Router registration:** `ai_tools.router` is added to `app/main.py` under the `/api/v1/ai/tools` prefix. The registration is conditional via feature flag (US-BKND-AI-002) or at minimum wrapped so disabling it leaves the app intact.
...
12. **Code review completed:** PR is reviewed by at least one other engineer. All review comments are addressed. The PR description references this story (US-BKND-AI-007).
...
|---|---|---|---|---|
| T-001 | **Implement ToolExecutor service** -- Create `app/services/ai/tool_executor.py` with `_handle_list_pages`, `_handle_fetch_page`, `_compute_page_etag`, `_derive_template_type`, `_error` helper, session validation (calling AISessionRepository stub), and audit logging stub. | Backend Engineer | 6 | US-BKND-AI-006 (AISessionRepository interface) |
