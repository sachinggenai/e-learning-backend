# US-BKND-AI-033: Event-Driven Outbox for Downstream Consumers
...
- **Depends on:** US-BKND-AI-010 (Apply Safety, Audit, and Outbox), US-BKND-AI-030 (Course Assembly into Editor)
- **Unlocks:** US-BKND-AI-034 (Durable Workflow Engine -- outbox events drive workflow triggers), US-BKND-AI-036 (Cost Tracking -- outbox events feed usage records), US-BKND-AI-041 (Async Preview Generation -- outbox events trigger preview rebuild), external webhook integrations
- **Source flow:** 13. Platform Runtime and Operations Flow (Phases 5-6 -- Event Publication), US-BKND-AI-013 Table `outbox_events` DDL reference
...
| **Preview Rebuild Trigger** (consumer) | Subscribes to page change events; triggers async preview generation (US-BKND-AI-041) |
...
| `test_apply_proposal_emits_event_atomically` | Create a proposal (US-BKND-AI-009), apply it. Verify the domain mutation AND the outbox event appear only after commit. Rollback the transaction and verify neither appears. |
...
|---|---|---|
| US-BKND-AI-009 (Generic Proposal Lifecycle) | DONE / IN PROGRESS | Mutation endpoints must exist to integrate outbox events into |
| US-BKND-AI-010 (Apply Safety, Audit) | DONE / IN PROGRESS | The apply-mutation pathway with transactional commit is the integration point |
| US-BKND-AI-030 (Course Assembly into Editor) | DONE / IN PROGRESS | Assembly endpoints are where page mutations happen |
...
| Dependency | Story | Why |
|---|---|---|
| US-BKND-AI-034 (Durable Workflow Engine) | SHOULD | Outbox events naturally trigger workflow steps |
| US-BKND-AI-041 (Async Preview Generation) | SHOULD | Outbox events trigger preview rebuilds |
...
---
The complete epic has been written to `C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-034_DURABLE_WORKFLOW_ENGINE.md`.

Here is a summary of the file and its structure:

**File:** `/c/Users/ADMIN/e-learning-backend/docs/AI_Implemenation/00_User_StoriesUseCases/US-BKND-AI-034_DURABLE_WORKFLOW_ENGINE.md`
