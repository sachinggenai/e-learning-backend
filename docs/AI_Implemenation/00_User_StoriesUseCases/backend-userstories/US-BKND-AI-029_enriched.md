## US-BKND-AI-029 — Batch Proposal and All-or-Nothing Semantics
...
| Field | Value |
|---|---|
| **Identifier** | US-BKND-AI-029 |
| **Epic Name** | Batch Proposal and All-or-Nothing Semantics |
| **Priority** | SHOULD for MVP, MUST for file-import MVP |
| **Depends on** | US-BKND-AI-010 (Apply safety, audit, outbox — transactional apply with idempotency keys) |
| **Unlocks** | US-BKND-AI-019 (Full course from uploaded file), US-BKND-AI-034 (Durable workflow engine) |
...
- File ingestion (US-BKND-AI-017, US-BKND-AI-019) requires batch creation of an entire course outline. Batch proposals are the atomic unit for this workflow.
...
- Outbox events are emitted for each individually applied page (reusing the existing outbox event types from US-BKND-AI-010: `PageCreatedByAI`, `PageUpdatedByAI`).
...
**Existing table that already exists (from US-BKND-AI-004):**
```sql
...
| T4 | **Implement `ProposalValidator`** in `app/services/ai/proposal_validator.py` — reuses existing `Course` model, component registry, and export validator to validate a single page proposal. | 4h | US-BKND-AI-008, US-BKND-AI-010 |
...
| T7 | **Implement `BatchProposalService.apply_batch`** — transactional apply loop, PageRecord/ComponentRecord creation, outbox event emission, idempotency support. | 8h | T3, T4, US-BKND-AI-010 (outbox) |
...
|---|---|---|
| US-BKND-AI-004 (AI persistence foundations) | Hard | Requires `ai_sessions` table for session_id FK |
| US-BKND-AI-010 (Apply safety, audit, outbox) | Hard | Batch apply reuses transactional apply + outbox patterns |
| US-BKND-AI-008 (Validation pipeline) | Hard | `ProposalValidator` reuses `Course` Pydantic model and component registry validation |
| US-BKND-AI-005 (Tool schemas and template contracts) | Hard | Template type allowlist checks |
...
| Story | Relationship |
|---|---|
| US-BKND-AI-019 (Full course from uploaded file) | Blocked by this story — file ingestion generates multiple pages that must be applied atomically |
| US-BKND-AI-034 (Durable workflow engine) | Consumes batch proposal as the apply step for long-running file ingestion workflows |
...
| **Stale course state** between proposal and apply | Medium | Apply step re-validates all pages within the transaction using base_hash comparison (from US-BKND-AI-009). |
...
---
The complete US-BKND-AI-030 epic has been written to:

**`C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-030_COURSE_ASSEMBLY_INTO_EDITOR.md`**
...
**Section 5 — Expansion Points:** Six post-MVP enhancements: batch assembly (US-BKND-AI-029), concurrent edit conflict resolution, undo/redo (US-BKND-AI-040), dynamic component type resolution, dry-run preview, page order auto-shift.
