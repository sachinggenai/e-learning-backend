# US-BKND-AI-017 -- Extract Documents and Review Proposed Page Breakdown
...
**Depends on:** US-BKND-AI-016 (File upload and ingestion job foundation), US-BKND-AI-008 (Unified course/page/schema/export/accessibility validation).
...
1. The author uploads a supported document (PDF, DOCX) via the AI ingestion endpoint (post-US-BKND-AI-016). The upload is validated for file type, size, malware, and stored to the configured storage backend.
...
8. The user clicks "Approve Plan" or "Cancel". Cancel returns the job to `staged` status (no data lost). Approve locks the plan and transitions the job to `plan_approved`, unlocking downstream US-BKND-AI-019 (content generation).
9. If auto-approval policies are configured (US-BKND-AI-032), low-risk documents (single section, content-text only, author is trusted role) may skip the review checkpoint.

### 1.2 Input / Output Contracts

**Input: ingestion job ID** (from US-BKND-AI-016 upload)
...
# --- Approve the page plan (advance to US-BKND-AI-019) ---
...
**Option B -- New `/api/v1/ai/ingestions` router** (for cleaner separation once AI module exists per US-BKND-AI-003):
...
Add a dedicated AI ingestion table rather than overloading the existing `import_jobs` table (which is SCORM-specific). This clean separation follows the pattern from US-BKND-AI-003 (isolated AI module).
...
CREATE TABLE ai_ingestion_jobs (
    ...
    ai_session_id   VARCHAR(64),                     -- FK to ai_sessions (US-BKND-AI-004)
...
```python
# New env vars for US-BKND-AI-017
...
If the durable workflow engine from US-BKND-AI-034 is available, the extract-and-segment pipeline runs as a `FileIngestionWorkflow` with checkpointed phases:
...
If US-BKND-AI-034 is not yet implemented, the pipeline runs synchronously (for files under 10 MB) or via a simple Celery/ARQ background task.
...
| **Observability** | LLM token usage per segmentation | Tracked and emitted | Cost tracker integration (US-BKND-AI-036) |
...
- **No AI-specific routers** exist yet (US-BKND-AI-003 not implemented). The plan-surface can be added to `app/routers/imports.py` as interim.

- **No AI session persistence** exists (US-BKND-AI-004 not implemented). Traceability will use job_id + user_id as fallback until sessions are available.
...
| Multi-document merging | Users uploading multiple files per course | US-BKND-AI-029 (batch proposals) | Merge multiple extractions into one page plan |
| Auto-approve for trusted documents | Admin efficiency requirements | US-BKND-AI-032 (policy engine), US-BKND-AI-002 (flags) | Deterministic rules skip human review |
| Extraction preview in frontend without LLM | Author wants to see raw sections first | Frontend AI integration (US-BKND-AI-024) | Show ExtractedDocument before segmentation |
...
| Custom template type overrides per section | Advanced authors | US-BKND-AI-005 (tool contracts) | Allow manual template assignment during review |
...
7. **Background Processing** -- Extraction runs in a background worker (either US-BKND-AI-034 durable workflow or simple background task). Job status progresses through `uploaded -> extracting -> segmenting -> plan_ready | plan_validation_failed`. Frontend polls the job endpoint for status updates.
...
- [ ] Integrate with model router (ModelRouter from US-BKND-AI-026 or direct provider call)
...
**Sub-tasks:**
- [ ] Integrate with US-BKND-AI-034 durable workflow engine if available, or implement a simple asyncio background task
...
- [ ] Update `docs/AI_Implemenation/00_User_StoriesUseCases/USER_STORIES.md` to mark US-BKND-AI-017 as enriched
...
# User Story US-BKND-AI-018: Display Course Validation Reports to Authors and Reviewers
...
### 1.3 Priority
SHOULD (MVP scope). Depends on US-BKND-AI-008 (validation pipeline).
...
| T-015 | Wire proposal validation into the apply pipeline (US-BKND-AI-010) so that `apply_*` endpoints reject with 422 when validation fails. | BE | 0.5 day |
...
**File created:** `C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-019_GENERATE_FULL_COURSE_FROM_FILE.md` (2,785 lines)
...
**`C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-020_ADMIN_AUDIT_COMPLIANCE_RECOVERY_EPIC.md`** (2,174 lines, 8 sections)
...
**Section 8 -- Task Breakdown:** 16 tasks across 6 groups (Foundation, Audit Read API, Recovery & Rollback, Configuration & Integration, Testing, Documentation) totaling 25 story points, with dependencies mapped to US-BKND-AI-004, US-BKND-AI-010, US-BKND-AI-009, US-BKND-AI-002, and US-BKND-AI-025.

---
The complete enriched US-BKND-AI-021 story has been written to:

**`C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-021_OBSERVABILITY_RATE_LIMITS_ROLLOUT_GATES.md`**
...
**File:** `C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-022_E2E_REGRESSION_RELEASE_READINESS.md`
...
# Orchestration & Platform (US-BKND-AI-023 — US-BKND-AI-032)

The enriched user story has been written to:

**`C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-023_AI_CHAT_ENDPOINT_LLM_INTERACTION_LOOP.md`**
...
**`C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-024_FRONTEND_AI_INTEGRATION_LAYER.md`** (2,163 lines)
...
**Section 4 -- Current State:** Inventory of existing backend AI endpoints (planned), frontend structure (`src/services/*`, `src/export-runtime/*`), feature flag system, error envelopes, and architecture documentation. Detailed gap analysis listing all 14+ missing deliverables. Dependency table mapping to US-BKND-AI-002 through US-BKND-AI-023.
...
**`C:\Users\ADMIN\e-learning-backend\docs\AI_Implemenation\00_User_StoriesUseCases\US-BKND-AI-025_PROMPT_SAFETY_CONTENT_GUARDRAILS.md`**
...
**Section 4: Current State** -- Assessment of existing codebase: no guardrail infrastructure exists, no `app/services/ai/` directory, no AI routers beyond US-BKND-AI-020 admin pattern, no PII libraries, no toxicity classifier, existing `error_envelope.py` and `feature_flags.py` available for reuse.
