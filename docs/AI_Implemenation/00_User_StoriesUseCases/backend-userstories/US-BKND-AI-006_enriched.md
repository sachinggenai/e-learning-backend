# US-BKND-AI-006 — Create Scoped AI Authoring Sessions

**Priority:** MUST  
**Depends on:** US-BKND-AI-004 (AI Persistence Foundations), US-BKND-AI-005 (Tool and Template Contract Registry)  
**Unlocks:** US-BKND-AI-007 (Page List and Fetch Tools), US-BKND-AI-008 (Validation Pipeline), US-BKND-AI-009 (Proposal Lifecycle), US-BKND-AI-023 (Chat Endpoint), US-BKND-AI-024 (Frontend AI Integration)  
...
#### 2.2.2 `ai_audit_logs` (extend from US-BKND-AI-004)

```sql
CREATE TABLE ai_audit_logs (
    -- Inherits from US-BKND-AI-004 foundation, adding session-specific fields:
    id                  BIGSERIAL PRIMARY KEY,
...
2. Add columns to `ai_audit_logs` if not already present from US-BKND-AI-004.
...
| `AIAuditLogger` | `app/services/ai/audit_logger.py` | New audit capabilities (or extend US-BKND-AI-004 foundation) |
...
| `app/models/ai_audit_log.py` (if not from US-BKND-AI-004) | ORM model | Audit persistence |
...
|---------|-------------|------------|------------|--------------|
| T-006-01 | **Create `AISessionRecord` ORM model** <br>- Define `app/models/ai_session.py` with all columns, constraints, and indexes <br>- Add `AISessionRecord` to `app/models/__init__.py` exports <br>- Add import in `app/main.py` lifespan <br>- Generate Alembic migration <br>- Run migration on dev DB and verify `CREATE TABLE` output | Backend Engineer (Data) | 2 | US-BKND-AI-004 (foundational models base) |
...
| T-006-05 | **Implement AI audit logger** <br>- Create `app/services/ai/audit_logger.py` with `AIAuditLogger` class <br>- Extend `ai_audit_logs` table schema if not already from US-BKND-AI-004 <br>- Integrate with `AISessionService` for create/get/delete events <br>- Test audit entry creation and querying | Backend Engineer | 2 | T-006-01, US-BKND-AI-004 |
...
- The frontend team can begin work on `US-BKND-AI-024` (Frontend AI Integration Layer) in parallel, as the API contracts in section 2.1 are the stable boundary.
- The `AISessionService` `expire_stale_sessions()` background job should be extracted as a dependency (`SessionExpiryService`) so it can be tested independently and reused by `US-BKND-AI-039` (Session Context Window Recovery).
