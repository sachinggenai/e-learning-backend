## US-BKND-AI-012: Propose and Apply AI Updates to Existing Pages

**Priority:** MUST
**Depends on:** US-BKND-AI-010 (Apply Safety, Audit, and Outbox)
**Unlocks:** US-BKND-AI-014 (Simple Chat Edit Orchestration)
...
**No AI-specific tables exist yet** — those will be created by US-BKND-AI-004 (ai_sessions, ai_proposals, ai_confirmation_tokens, ai_audit_logs, outbox_events). US-BKND-AI-012 consumes the proposal and apply infrastructure from US-BKND-AI-009 and US-BKND-AI-010.
...
6. `component_type` (on add) is in the supported whitelist from US-BKND-AI-005 tool contract registry.
7. If `components` patch results in zero components, the proposal is allowed but flagged with a warning.
8. The resulting in-memory page passes the validation pipeline (US-BKND-AI-008).
...
1. **Page fetch tool** (`fetch_page` from US-BKND-AI-007) returns `page_hash` in the response metadata.
...
- `proposal_apply_service.py` — Apply logic with transactional safety (shared with US-BKND-AI-011, US-BKND-AI-013)
...
**Modified files:**
- `app/repositories/ai_proposal_repo.py` — New repository for `ai_proposals` table (created by US-BKND-AI-004, extended here for update-specific queries)
- `app/services/ai/session_service.py` — Add session validation helper (created by US-BKND-AI-006, extended here)

**New API endpoints (under `/api/v1/ai/`):**
- None directly; the tool-call adapters are invoked via the chat orchestrator (US-BKND-AI-023). However, for testing and direct integration, we expose:
...
#### 3.2 Database DDL (New AI Tables, from US-BKND-AI-004)

These tables are created by US-BKND-AI-004 but referenced here. The migration scripts live in `alembic/versions/`. The full DDL is included here for clarity.

**`ai_sessions`** (created by US-BKND-AI-004):
```sql
...
**`ai_proposals`** (created by US-BKND-AI-004, extended by US-BKND-AI-012):
```sql
...
**`ai_audit_logs`** (created by US-BKND-AI-004):
```sql
...
**`outbox_events`** (created by US-BKND-AI-004):
```sql
...
- `xxxx_ai_sessions.py` (US-BKND-AI-004)
- `xxxx_ai_proposals.py` (US-BKND-AI-004, extended by US-BKND-AI-009)
- `xxxx_ai_update_specific_indices.py` (US-BKND-AI-012 adds page_id, base_hash indices)
- `xxxx_ai_audit_logs.py` (US-BKND-AI-004)
- `xxxx_outbox_events.py` (US-BKND-AI-004)
...
class ProposalApplyService:
    ...
    async def apply(
        ...
        """
        Shared apply logic used by US-BKND-AI-011 (create), US-BKND-AI-012 (update),
        and US-BKND-AI-013 (delete).
        Handles:
...
revision = "xxxx"
down_revision = "<previous>"  # Must point to US-BKND-AI-004 migration
...
6. **Validation integration**: The proposal validates the in-memory patched page through the US-BKND-AI-008 validation pipeline. Blocking errors are returned in the proposal preview; apply is blocked if blocking errors remain unresolved.
...
2. **RLHF feedback integration**: Capture whether the author accepted, modified, or rejected the proposal as implicit feedback for model improvement. See US-BKND-AI-031.

3. **Auto-apply for low-risk changes**: Allow the policy engine (US-BKND-AI-032) to auto-apply simple content-only patches for trusted roles without manual confirmation.

4. **Multi-page batch update**: Extend the update proposal to accept an array of `{page_id, patch}` for batch updates with all-or-nothing semantics. See US-BKND-AI-029.

5. **Preview generation**: Generate an HTML preview of the patched page asynchronously so the author can see the rendered result before approving. See US-BKND-AI-041.
...
7. **Rollback**: Store the `before_snapshot` in a way that enables automatic rollback proposal generation. See US-BKND-AI-040.
...
- List pages (US-BKND-AI-007 tool), pick one.
...
|---|---|---|---|---|
| US-BKND-AI-012-T01 | Create `app/services/ai/page_hash.py` with `compute_page_hash(page: PageRecord) -> str` and `compute_page_hash_from_dict(page: dict) -> str`. Write unit tests. | Backend | None | 3h |
| US-BKND-AI-012-T02 | Create `app/services/ai/diff_computer.py` with `DiffComputer.compute_diff(before, after)` and `DiffComputer.compute_page_hash()`. Write unit tests. | Backend | T01 | 4h |
| US-BKND-AI-012-T03 | Create `app/services/ai/patch_applicator.py` with `PatchApplicator.apply_patch(page, patch)` and `validate_patch_fields(patch)`. Write unit tests for all component operations. | Backend | None | 5h |
| US-BKND-AI-012-T04 | Create `app/services/ai/proposal_schemas.py` with all Pydantic request/response schemas. | Backend | None | 2h |

#### Chunk 1: Core Service Logic (Days 3-5)

| Task ID | Description | Owner | Dependencies | Effort |
|---|---|---|---|---|
| US-BKND-AI-012-T05 | Create `app/services/ai/page_update_service.py` with `PageUpdateService.propose_update()` method: session validation, hash check, patch apply in memory, validation pipeline call, diff compute, proposal persistence. Write integration tests. | Backend | T02, T03, T04, US-BKND-AI-006 (session validation), US-BKND-AI-008 (validation pipeline) | 8h |
| US-BKND-AI-012-T06 | Create `app/services/ai/proposal_apply_service.py` with `ProposalApplyService.apply()`: idempotency check, re-validation, transactional apply across PageRepository + ComponentRepository, audit write, outbox write. Write integration tests. | Backend | T05, US-BKND-AI-010 (audit/outbox contracts) | 8h |
| US-BKND-AI-012-T07 | Create `app/repositories/ai_proposal_repo.py` with CRUD operations for `ai_proposals` table. | Backend | US-BKND-AI-004 (ai_proposals model) | 3h |

#### Chunk 2: API Layer + Feature Flags (Days 5-6)

| Task ID | Description | Owner | Dependencies | Effort |
|---|---|---|---|---|
| US-BKND-AI-012-T08 | Create tool-call adapter functions in `app/services/ai/tool_adapters.py`: `handle_propose_update_page(params)` and `handle_apply_update_proposal(params)`. These validate input, call the service, and format the response. | Backend | T05, T06, US-BKND-AI-005 (tool contract registry) | 4h |
| US-BKND-AI-012-T09 | Register `propose_update_page` and `apply_update_proposal` in the AI tool registry (from US-BKND-AI-005). Wire up feature flag checks. | Backend | T08, US-BKND-AI-005 | 2h |
| US-BKND-AI-012-T10 | Create `app/services/ai/config.py` for AI configuration. Add `AI_FEATURE_PAGE_UPDATE` and related env vars to `.env.example`. Update app startup to validate config. | Backend | None | 2h |

#### Chunk 3: Alembic Migration (Day 6)

| Task ID | Description | Owner | Dependencies | Effort |
|---|---|---|---|---|
| US-BKND-AI-012-T11 | Create Alembic migration for US-BKND-AI-004 / US-BKND-AI-009 foundational tables (`ai_sessions`, `ai_proposals`, `ai_audit_logs`, `outbox_events`) — shared migration that US-BKND-AI-012 builds on. | Backend | US-BKND-AI-004 | 3h |
| US-BKND-AI-012-T12 | Add indices migration specifically for US-BKND-AI-012 query patterns (page_id lookups, idempotency key lookups, status-based queries). | Backend | T11 | 1h |
| US-BKND-AI-012-T13 | Verify migration up/down against PostgreSQL. Update `alembic/env.py` if needed to include new models. | Backend | T12 | 2h |

#### Chunk 4: Testing (Days 7-8)

| Task ID | Description | Owner | Dependencies | Effort |
|---|---|---|---|---|
| US-BKND-AI-012-T14 | Write unit test suite for `patch_applicator.py` (all operations, edge cases). | Backend | T03 | 3h |
| US-BKND-AI-012-T15 | Write unit test suite for `diff_computer.py` (all diff scenarios). | Backend | T02 | 2h |
| US-BKND-AI-012-T16 | Write unit test suite for `page_hash.py` (determinism, sensitivity). | Backend | T01 | 1h |
| US-BKND-AI-012-T17 | Write integration tests for propose+apply happy path, stale hash, out-of-scope fields, idempotency, expiry, concurrent edit detection. | Backend | T05, T06 | 6h |
| US-BKND-AI-012-T18 | Write E2E test: create course -> session -> propose update -> preview -> apply -> verify persistence + audit + outbox. | Backend | T17 | 3h |
| US-BKND-AI-012-T19 | Add test for feature-flag-disabled path (503 response). | Backend | T10, T09 | 1h |

#### Chunk 5: Documentation and Sign-Off (Day 8)

| Task ID | Description | Owner | Dependencies | Effort |
|---|---|---|---|---|
| US-BKND-AI-012-T20 | Update `TOOL_SCHEMAS_CLAUDE_NATIVE.md` with `propose_update_page` and `apply_update_proposal` tool definitions. | Tech Writer | T08 | 2h |
| US-BKND-AI-012-T21 | Update `BACKEND_IMPLEMENTATION_SUMMARY.md` with update proposal flow overview. | Tech Writer | T05, T06 | 1h |
| US-BKND-AI-012-T22 | Populate `docs/AI_Implemenation/02DetailedDesign/US-BKND-AI-012.md` with the design document. | Architect | All | 2h |
| US-BKND-AI-012-T23 | Sign-off: verify all acceptance criteria against integration test results. | PO/QA | T17, T18, T19 | 2h |
...
`/c/Users/ADMIN/e-learning-backend/docs/AI_Implemenation/00_User_StoriesUseCases/US-BKND-AI-013_DELETE_PAGE_EPIC.md`
...
**5. Expansion Points** — Soft-delete retention, batch delete proposals, automatic reorder optimization, UI-initiated delete proposals, policy engine integration (US-BKND-AI-032), and preview-before-delete (US-BKND-AI-041).
