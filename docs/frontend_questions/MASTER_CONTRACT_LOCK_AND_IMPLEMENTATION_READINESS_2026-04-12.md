...
## 17. Implementation Execution Plan (Completed)

This section converts contract intent into executable work with fixed ownership and acceptance checks.

### 17.1 Execution Windows

1. Window A (Apr 12-Apr 14): Contract activation and environment readiness.
2. Window B (Apr 15-Apr 19): Backend P0 delivery and package evidence.
3. Window C (Apr 20-Apr 26): Phase 1 MVP completion and staging validation.
4. Window D (Apr 27-May 10): Extended templates and error envelope adoption.
5. Window E (May 11-May 24): Hardening, accessibility, LMS validation, release.

### 17.2 Work Breakdown by Owner

Backend AI Developer:

1. Add `exportContractVersion` to `course_data.js` export payload by Apr 15.
2. Add `supportedTemplateTypes[]` to `course_data.js` export payload by Apr 15.
3. Deliver two fresh exported packages (assessment-heavy and branching-heavy) by Apr 19.
4. Standardize error envelope across scoring/completion/export by Apr 22.
5. Publish canonical API examples matching production behavior by Apr 26.

Frontend AI Developer:

1. Lock all production API calls to canonical `/api/v1` routes only.
2. Complete Phase 1 components (accordion, tabs, MCQ, progress, fallback renderer) by Apr 26.
3. Implement contract smoke tests and pass against live backend.
4. Treat new export fields as optional until Apr 15, required for new packages after Apr 15.
5. Add retry-safe handling for scoring/completion submissions.

QA:

1. Validate endpoint contracts (status codes + payload schema).
2. Validate exported package structure and runtime dispatch behavior.
3. Validate unsupported template fallback behavior.
4. Validate suspend/resume and state restoration in LMS harness.

TPO:

1. Run daily blocker sync during Window B and Window C.
2. Run milestone gate review every Wednesday.
3. Enforce go/no-go decisions when mandatory criteria fail.

### 17.3 Day-Level Delivery Plan (Apr 12-Apr 26)

1. Apr 12: Sign-off circulation, canonical endpoint freeze, start FE service integration.
2. Apr 13: FE contract smoke test scaffolding, BE P0 field implementation in progress.
3. Apr 14: FE MVP component wiring start, BE internal export payload verification.
4. Apr 15: BE ships export payload fields (`exportContractVersion`, `supportedTemplateTypes[]`).
5. Apr 16: FE validates new fields and updates parser behavior.
6. Apr 17: FE completes scoring/completion integration pass and retry logic pass.
7. Apr 18: BE prepares two sample packages from latest branch.
8. Apr 19: Sample packages delivered and jointly validated.
9. Apr 20: FE contract smoke suite run against sample package flows.
10. Apr 21: FE fallback UX verification for unsupported template types.
11. Apr 22: BE ships normalized error envelope.
12. Apr 23: FE adopts normalized envelope adapter and runs regression tests.
13. Apr 24: LMS harness test (suspend/resume + completion + scoring).
14. Apr 25: Full Phase 1 regression pass, accessibility baseline check.
15. Apr 26: Phase 1 staging sign-off decision.

### 17.4 Mandatory Gates

Gate 1 (Apr 15):

1. Export payload includes both required metadata fields.

Gate 2 (Apr 19):

1. Both sample packages are delivered and validated.

Gate 3 (Apr 22):

1. Error envelope normalization is available for integration.

Gate 4 (Apr 26):

1. Phase 1 MVP passes smoke, fallback, and staging checks.

### 17.5 Completion Criteria (Plan Done)

This implementation plan is considered complete only when all are true:

1. All owners accepted tasks and dates.
2. All mandatory gates passed on or before due dates.
3. Canonical contract remained unchanged in implementation.
4. No FE production path depends on legacy/alias endpoint routing.
5. TPO signs final execution readiness for Phase 2 continuation.

### 17.6 Execution Notes

1. If a gate fails, implementation continues only in non-release mode until corrected.
2. Any contract change request must be approved jointly by FE, BE, and TPO before merge.
3. This section is operational and supersedes informal chat updates for planning decisions.
